...
import Main.CharacterResources.Items.StoneHeartItem;
import Main.CharacterResources.Items.WhetstoneItem;
import Main.CharacterResources.Items.WingBootsItem;
import Main.CharacterResources.Player.Classes.PlayerIceMage;
 ...
 */
public class GamePlayComponent extends JComponent implements KeyListener {
    ...
    private final HashMap<Integer, Room> roomRef = new HashMap<Integer,Room>();
    private static final int DEFAULT_ROOM = 24;


    //Out of house variables
    private javax.swing.Timer timer;    //For Thread Management
    /**
    ...
    public GamePlayComponent(int inWidth, int inHeight, MainFrame parent)
    {
        ...
        System.out.println(timer.getActionListeners());
        setFocusable(true);
        requestFocus();
    }
    ...
    public void traverseStairs(Stairs stairs)
    {
        if(player.collides(stairs) != null)
        {
            ...
            {
                case NORTH:
                    player.setPos(player.getX(),getPreferredSize().height - stairs.getHeight()- Main.Collision.Collideable.PIXEL_CONSTANT-5);
                    break;
                case SOUTH:
